    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          key={star}
          className={cn(
            'h-4 w-4 md:h-5 md:w-5', // Increased star size, even bigger on desktop
...
export default function HeroTestimonials() {
  return (
    <div className="mt-16 md:mt-20"> {/* Increased top margin */}
      {/* Platform ratings */}
      <div className="flex flex-wrap justify-center gap-8 md:gap-10 items-center"> {/* Increased gap */}
        {testimonialStats.featuredPlatforms.map((platform) => (
          <div key={platform.name} className="text-center flex flex-col items-center">
            ...
            <p className="mt-2 text-base text-muted-foreground"> {/* Increased text size and margin */}
